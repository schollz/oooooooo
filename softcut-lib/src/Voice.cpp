...
Voice::Voice()
    : rateRamp(48000, 0.1), preRamp(48000, 0.1), recRamp(48000, 0.1) {
  svfPreFcBase = 16000;
  reset();
}
...
void Voice::setLoopStart(float sec) { sch.setLoopStartSeconds(sec); }

void Voice::setLoopEnd(float sec) { sch.setLoopEndSeconds(sec); }

void Voice::setFadeTime(float sec) { sch.setFadeTime(sec); }

void Voice::cutToPos(float sec) { sch.cutToPos(sec); }

void Voice::setRecLevel(float amp) { recRamp.setTarget(amp); }

void Voice::setPreLevel(float amp) { preRamp.setTarget(amp); }
...
void Voice::setLoopFlag(bool val) { sch.setLoopFlag(val); }
...
void Voice::setPreFilterRq(float x) { svfPre.setRq(x); }

void Voice::setPreFilterLp(float x) { svfPre.setLpMix(x); }

void Voice::setPreFilterHp(float x) { svfPre.setHpMix(x); }

void Voice::setPreFilterBp(float x) { svfPre.setBpMix(x); }

void Voice::setPreFilterBr(float x) { svfPre.setBrMix(x); }

void Voice::setPreFilterDry(float x) { svfPreDryLevel = x; }

void Voice::setPreFilterFcMod(float x) { svfPreFcMod = x; }

void Voice::updatePreSvfFc() {
  float fcMod =
      std::min(svfPreFcBase,
               svfPreFcBase * std::fabs(static_cast<float>(sch.getRate())));
  fcMod = svfPreFcBase + svfPreFcMod * (fcMod - svfPreFcBase);
  svfPre.setFc(fcMod);
}

// output filter
void Voice::setPostFilterFc(float x) { svfPost.setFc(x); }

void Voice::setPostFilterRq(float x) { svfPost.setRq(x); }

void Voice::setPostFilterLp(float x) { svfPost.setLpMix(x); }

void Voice::setPostFilterHp(float x) { svfPost.setHpMix(x); }

void Voice::setPostFilterBp(float x) { svfPost.setBpMix(x); }

void Voice::setPostFilterBr(float x) { svfPost.setBrMix(x); }

void Voice::setPostFilterDry(float x) { svfPostDryLevel = x; }
...
void Voice::setRateSlewTime(float d) { rateRamp.setTime(d); }

void Voice::setPhaseQuant(float x) { phaseQuant = x; }

void Voice::setPhaseOffset(float x) { phaseOffset = x * sampleRate; }
...
void Voice::updateQuantPhase() {
  if (phaseQuant == 0) {
    quantPhase.store(sch.getActivePhase() / sampleRate,
                     std::memory_order_relaxed);
  } else {
    const phase_t tmp =
        (sch.getActivePhase() + phaseOffset) / (sampleRate * phaseQuant);
    quantPhase.store(std::floor(tmp) * phaseQuant, std::memory_order_relaxed);
  }
}

bool Voice::getPlayFlag() { return playFlag; }

bool Voice::getRecFlag() { return recFlag; }
...
float Voice::getSavedPosition() {
  return static_cast<float>(rawPhase.load(std::memory_order_relaxed) /
                            sampleRate);
}

void Voice::stop() { sch.stop(); }
