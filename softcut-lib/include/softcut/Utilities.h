...
class LogRamp {
 ...
 public:
  explicit LogRamp(float sr = 48000, float t = 0.05)
      : sampleRate(sr), b(1.f), x0(0.f), y0(0.f) {
    sampleRate = sr;
    time = t;
    setTime(t);
  }
  ...
  // get value
  float getValue() const { return y0; }
  ...
  // in this case, _t_ is the time for the filter to converge within -60dB of
  // target
  void setTime(float t) {
    time = t;
  ...
  float getTarget() const { return x0; }
...
class Slew {
 ...
 public:
  Slew(float sr, float rise = 0.2, float fall = 0.2) : sampleRate(sr), x0(0.f) {
    setRiseTime(rise);
    setFallTime(fall);
  }
