 ...
 private:
  ...
  void preProcess(jack_nframes_t numFrames) {
    ...
    for (int i = 0; i < NumIns / 2; ++i) {
      source[i][0] = static_cast<const float*>(
          jack_port_get_buffer(inPort[j++], numFrames));
      source[i][1] = static_cast<const float*>(
          jack_port_get_buffer(inPort[j++], numFrames));
    }
    j = 0;
 ...
 public:
  virtual void setup() {
    ...
    for (int i = 0; i < NumIns; ++i) {
      ...
      inPort[i] = jack_port_register(client, name, JACK_DEFAULT_AUDIO_TYPE,
                                     JackPortIsInput, 0);
      if (inPort[i] == nullptr) {
    ...
    for (int i = 0; i < NumOuts; ++i) {
      ...
      os << "output_" << (i + 1);
      const char* name = os.str().c_str();
      outPort[i] = jack_port_register(client, name, JACK_DEFAULT_AUDIO_TYPE,
                                      JackPortIsOutput, 0);
  ...
  void cleanup() { jack_client_close(client); }

 public:
  // NB: alas, we can't call setup() from c-tor because it includes a virtual
  ...
  void stop() { jack_deactivate(client); }
  ...
  //---- getters
  const char* getInputPortName(int idx) { return jack_port_name(inPort[idx]); }
  ...
  // FIXME: surely there is a cleaner way to use templated class reference
  // parameter, here
  template <int N, int M>
  bool connect(JackClient<N, M>* other, int sinkIdx, int sourceIdx) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      if (jack_connect(this->client, srcPortName, dstPortName)) {
        std::cerr << "Client::connect(): port failed for channel " << i
                  << std::endl;
        return false;
      }
    }
